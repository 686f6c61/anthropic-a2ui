...
def create_a2ui_tool(
  ...
  """
  if allowed_components is not None or allowed_messages is not None:
    catalog = catalog.with_pruning(
...
def validate_tool_input(
  ...
  Args:
    ...
    input_json: El objeto (dict o lista) que Claude devolvio como
      ``a2ui_json``.
  ...
  if repair:
    ...
    if isinstance(payload, list):
      payload = repair_childlists(payload)
      payload = repair_orphans(payload)
      payload = repair_icons(payload)
      payload = repair_functions(payload)
    else:
      payload = repair_childlists([payload])
      payload = repair_orphans(payload)
      payload = repair_icons(payload)
      payload = repair_functions(payload)
      payload = payload[0] if payload else input_json
